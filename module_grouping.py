...
def find_relative(filted, interval=0.2):
    ...
    for k in found_key_list:
        new_value = find_key(k[0])
        if new_value is not None:
            value_list.append(new_value[0])
            
    #print("value=", value_list)
    group_list = [{"name":n[0], "top":t}for n, t in zip(found_key_list, value_list)]
    group_dict = {"name":keys[target_index][0], "top":find_key(keys[target_index])}
...
def get_corr_json():
    ...
    while now_level > 0.1:
        ...
        while now_level > lower_bound:
            now_level, g = find_relative(filted_corr, interval)
            children.append(g)
